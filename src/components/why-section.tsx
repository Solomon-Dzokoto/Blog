  ...
  return (
    ...
    >
        <div className="flex-col gap-6 max-w-[757px] flex">
          <h2
            id="why-choose-us"
            className="font-extrabold text-3xl md:text-[2.5rem] leading-10 md:leading-12  text-center"
          >
            Why Choose Our AI-Powered Outbound Calling Solution?
          </h2>
          <p className="md:text-base text-sm leading-6 text-center text-[#53686A]">
           ...
           <div className="grid grid-cols-1 lg:grid-cols-3 lg:grid-rows-2 justify-between gap-x-7 gap-y-8 md:gap-y-10 p-4">
              {
                whyCardArr.map(card => (
                  <article key={card.title} className="flex flex-col items-center md:items-start
                  rounded-3xl bg-secondary-variant p-8  gap-4 md:gap-8">
                    <h3 className="font-bold text-2xl md:text-[2rem]  text-center md:text-start">
                      {card.title}
                    </h3>
                   <p className="md:text-base text-sm text-center md:text-start leading-6 text-secondary">
                    {
                      card.text
                    }
                   </p>
