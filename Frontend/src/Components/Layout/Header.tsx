  ...
  return (
    <header className=" shadow-lg p-4 flex justify-between items-center">
      ...
      <div className="flex items-center gap-4">
        ...
        <span
          className={`p-4 w-25 h-15 rounded-full flex justify-center items-center`}
        >
          {user ? (
            <img
              src={userLogo}
            ...
            <span className="cursor-pointer" onClick={toSignUp}>
              <CiUser />
            </span>
          )}
        </span>
        {user && (
          <button
            className="flex items-center gap-1.5 bg-[#032b13] cursor-pointer text-white transition pointer hover:opacity-95 w-full px-4 rounded-md py-2"
            onClick={() => {
              removeUser();
              navigate("/");
            }}
